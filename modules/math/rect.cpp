...
{
    return position.x < rect.position.x + rect.size.x && position.x + size.x > rect.position.x && position.y < rect.position.y + rect.size.y && position.y + size.y > rect.position.y;
}
...
Rect Rect::intersection(const Rect& rect) const
{
    ...
    Vector2 p2 = Vector2(std::min(position.x + size.x, rect.position.x + rect.size.x), std::min(position.y + size.y, rect.position.y + rect.size.y));
...
Rect Rect::unionBetween(const Rect& rect) const
{
    ...
    Vector2 p2 = Vector2(std::max(position.x + size.x, rect.position.x + rect.size.x), std::max(position.y + size.y, rect.position.y + rect.size.y));
    return Rect(p1, p2 - p1);
}
